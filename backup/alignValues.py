...
ALL_SUPPORTED_VALUES = ["humor", "gpt2-helpful", "gpt2-harmless", "diversity", "coherence", "perplexity"] # hardcode to reduce import time
import numpy as np
import csv
import os
...
class AlignValues:
    ...
    def find_pareto_by_interpolation(self, c_low, c_high):
        ...
        Args:
            c_low (list or torch.Tensor): Lower bound constraint vector.
            c_high (list or torch.Tensor): Upper bound constraint vector.

        Returns:
            tuple: (success (1 or 0), optimized c, optimized lambda) if successful; None otherwise.
        """
        ...
        while torch.norm(c_high - c_low) > 1e-3:  # Continue until the range is sufficiently small
            ...
            self.c = c_mid.clone()  # Ensure `c_mid` is cloned to prevent accidental overwriting
        ...
        if best_rho is not None:
            print(f"Feasible solution found with rho = {best_rho:.3f}")
            self._save_results_to_text(best_lambda, True)
            success_rate = success_count / iteration_count * 1.0
            return success_rate, best_c.tolist(), best_lambda

        print("Feasible solution not found.")
        return 0, None, None

    def find_pareto_by_interpolation_gridsearch(self, c_low, c_high, grid_points=10):
        """
        Perform sequential grid search to find the feasible palette c from c_low to c_high.

        Args:
            c_low (list or torch.Tensor): Lower bound constraint vector.
            c_high (list or torch.Tensor): Upper bound constraint vector.
            grid_points (int): Number of grid points to divide the line segment.

        Returns:
            tuple: (success_rate, optimized c, optimized lambda) if successful; None otherwise.
        """
        if not isinstance(c_low, (list, tuple, torch.Tensor)):
            c_low, c_high = [c_low], [c_high]
        c_low = torch.tensor(c_low, dtype=torch.float32)
        ...
        grid = [c_low + (i / (grid_points - 1)) * (c_high - c_low) for i in range(grid_points)]
        best_c = None
        best_lambda = None
        success_count = 0
        iteration_count = 0
        ...
        for c in grid:
            iteration_count += 1
            self.c = c.clone()  # Set current palette
            ...
            if success:
                ...
                print(f"Infeasible solution encountered at grid point {iteration_count}. Stopping search.")
                break  # Stop further grid search if infeasibility is encountered

        if best_c is not None:
            print(f"Feasible solution found after {success_count} feasible grid points.")
            self._save_results_to_text(best_lambda, True)
    ...
    def find_pareto_by_interpolation_gridsearch(self, c_low, c_high):
        """
        Perform sequential grid search to find the feasible palette c on the line between c_low and c_high.
        ...
        # Grid of rho values from 0.1 to 1.0 in steps of 0.1
        rho_values = torch.linspace(0.0, 1.0, 10)

        for rho in rho_values:
            # Compute the intermediate palette using the grid search
            c_mid = c_low + rho * (c_high - c_low)
            self.c = c_mid.clone()  # Clone to ensure immutability

            # Optimize for the current palette
            optimized_lambda, success = self.optimize_lambda(verbose=False)
            if success:
                success_count += 1
                # Update best solution
                ...
                # Stop further exploration if infeasible
                break

        if best_c is not None:
            print(f"Feasible solution found with rho = {rho:.3f}")
            self._save_results_to_text(best_lambda, True)
